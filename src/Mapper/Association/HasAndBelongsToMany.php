...
class HasAndBelongsToMany extends AbstractAssociation
{
    ...
    {
        $lookupTable = 'blogEntriesCategories';
        ...
        $query->join($lookupTable, 'blogEntriesCategories.categoryID = categories.categoryID', []);
        $query->where([$foreignKey => $sourceModel->$foreignKey]);
